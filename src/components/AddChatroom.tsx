...
function AddChatroom() {
  ...
  const handleCreateChatroom = async () => {
    await createChatroom({
      ...
      onCompleted: (data) => {
        console.log("data from handleCreateChatroom", data);
  ...
  //refetches users search upon debounced search input change
  const [searchTerm, setSearchTerm] = useState("");
  const { data, refetch } = useQuery<SearchUsersQuery>(SEARCH_USERS, {
    variables: { fullname: searchTerm },
  });
  console.log("data from AddChatroom: ", data);
  let debounceTimeout: NodeJS.Timeout;
  const handleSearchChange = (term: string) => {
    console.log("term from handleSearchChange: ", term);
    ...
    // If the code is executed again within the 300ms window, the previous timeout is cleared, and a new one is set.
  ...
  return (
    <Modal opened={isCreateRoomModalOpen} onClose={toggleCreateRoomModal}>
      <Stepper active={active} onStepClick={setActive} breakpoint="sm">
        {/* FIX IT. First step must be creation of a room, second - adding users */}
